...
"[project]/node_modules/@upstash/workflow/chunk-RP7G4UD5.mjs [app-rsc] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
__turbopack_context__.s({
    ...
    "makeGetWaitersRequest": ()=>makeGetWaitersRequest,
    "makeNotifyRequest": ()=>makeNotifyRequest,
    "prepareFlowControl": ()=>prepareFlowControl,
    "serve": ()=>serve,
    "serveBase": ()=>serveBase,
    "serveManyBase": ()=>serveManyBase,
    "triggerFirstInvocation": ()=>triggerFirstInvocation
});
// src/error.ts
...
;
var WorkflowError = class extends __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$qstash$2f$chunk$2d$H3E2J2K6$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["QstashError"] {
    constructor(message){
        super(message);
...
var fetchWithContextCall = async (context, agentCallParams, ...params)=>{
    ...
    try {
        ...
        const responseHeaders = new Headers(Object.entries(responseInfo.header).reduce((acc, [key, values])=>{
            acc[key] = values.join(", ");
            return acc;
        }, {}));
...
var createWorkflowModel = ({ context, provider, providerParams, agentCallParams })=>{
    return provider({
        fetch: (...params)=>fetchWithContextCall(context, agentCallParams, ...params),
        ...providerParams
    });
};
var wrapTools = ({ context, tools })=>{
    return Object.fromEntries(Object.entries(tools).map((toolInfo)=>{
...
var convertLangchainTool = (langchainTool)=>{
    return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ai$2f$dist$2f$index$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__$3c$locals$3e$__["tool"])({
...
var serveManyBase = ({ workflows, getUrl, serveMethod, options })=>{
    const workflowIds = [];
    const workflowMap = Object.fromEntries(Object.entries(workflows).map((workflow)=>{
        const workflowId = workflow[0];
        if (workflowIds.includes(workflowId)) {
            throw new WorkflowError(`Duplicate workflow name found: '${workflowId}'. Please set different workflow names in serveMany.`);
        }
        if (workflowId.includes("/")) {
            throw new WorkflowError(`Invalid workflow name found: '${workflowId}'. Workflow name cannot contain '/'.`);
        }
...
var getSteps = async (requester, workflowRunId, messageId, debug)=>{
    try {
        ...
        if (error instanceof __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$qstash$2f$chunk$2d$H3E2J2K6$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["QstashError"] && error.status === 404) {
...
function nanoid() {
    ...
    }).map(()=>NANOID_CHARS[getRandomInt()]).join("");
}
...
var triggerFirstInvocation = async (params)=>{
    const firstInvocationParams = Array.isArray(params) ? params : [
        params
    ];
    const workflowContextClient = firstInvocationParams[0].workflowContext.qstashClient;
    const invocationBatch = firstInvocationParams.map(({ workflowContext, useJSONContent, telemetry, invokeCount, delay })=>{
    ...
    try {
        ...
        const hasAnyDeduplicated = invocationStatuses.some((status)=>status === "workflow-run-already-exists");
...
var triggerRouteFunction = async ({ onCleanup, onStep, onCancel, debug })=>{
    try {
        ...
        const error_ = error;
        if (error instanceof __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$qstash$2f$chunk$2d$H3E2J2K6$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["QstashError"] && error.status === 400) {
...
var triggerWorkflowDelete = async (workflowContext, result, debug, cancel = false)=>{
    ...
    await debug?.log("SUBMIT", "SUBMIT_CLEANUP", `workflow run ${workflowContext.workflowRunId} deleted.`);
};
...
var handleThirdPartyCallResult = async ({ request, requestPayload, client, workflowUrl, failureUrl, retries, retryDelay, telemetry, flowControl, debug })=>{
    try {
        if (request.headers.get("Upstash-Workflow-Callback")) {
            ...
            if (requestPayload) {
                ...
                if (!workflowRunId2) throw new WorkflowError("workflow run id missing in context.call lazy fetch.");
                if (!messageId) throw new WorkflowError("message id missing in context.call lazy fetch.");
                const { steps, workflowRunEnded } = await getSteps(client.http, workflowRunId2, messageId, debug);
                if (workflowRunEnded) {
                    return ok("workflow-ended");
                }
                const failingStep = steps.find((step)=>step.messageId === messageId);
                if (!failingStep) throw new WorkflowError("Failed to submit the context.call. " + (steps.length === 0 ? "No steps found." : `No step was found with matching messageId ${messageId} out of ${steps.length} steps.`));
            ...
            if (!(callbackMessage.status >= 200 && callbackMessage.status < 300) && callbackMessage.maxRetries && callbackMessage.retried !== callbackMessage.maxRetries) {
                ...
                console.warn(`Workflow Warning: "context.call" failed with status ${callbackMessage.status} and will retry (retried ${callbackMessage.retried ?? 0} out of ${callbackMessage.maxRetries} times). Error Message:
${atob(callbackMessage.body ?? "")}`);
        ...
        return err(new WorkflowError(`Error when handling call return (isCallReturn=${isCallReturn}): ${error}`));
    }
};
...
var verifyRequest = async (body, signature, verifier)=>{
    ...
    try {
        ...
        throw new WorkflowError(`Failed to verify that the Workflow request comes from QStash: ${error}
...
var BaseLazyStep = class _BaseLazyStep {
    ...
    constructor(stepName){
        if (!stepName) {
            throw new WorkflowError("A workflow step name cannot be undefined or an empty string. Please provide a name for your workflow step.");
        }
        if (typeof stepName !== "string") {
            console.warn("Workflow Warning: A workflow step name must be a string. In a future release, this will throw an error.");
        }
        this.stepName = stepName;
    }
    /**
   ...
   */ parseOut(out) {
        if (out === void 0) {
            if (this.allowUndefinedOut) {
                ...
                throw new WorkflowError(`Error while parsing output of ${this.stepType} step. Expected a string, but got: undefined`);
            }
        }
        if (typeof out === "object") {
            if (this.stepType !== "Wait") {
                console.warn(`Error while parsing ${this.stepType} step output. Expected a string, but got object. Please reach out to Upstash Support.`);
                return out;
            }
            return {
                ...out,
        ...
        if (typeof out !== "string") {
            throw new WorkflowError(`Error while parsing output of ${this.stepType} step. Expected a string or undefined, but got: ${typeof out}`);
        }
...
var LazyCallStep = class _LazyCallStep extends BaseLazyStep {
    ...
    getHeaders({ context, telemetry, invokeCount, step }) {
        const { headers, contentType } = super.getHeaders({
            context,
            telemetry,
            invokeCount,
            step
        ...
        const forwardedHeaders = Object.fromEntries(Object.entries(this.headers).map(([header, value])=>[
...
var LazyWaitForEventStep = class extends BaseLazyStep {
    ...
    getBody({ context, step, headers, telemetry }) {
        ...
        const timeoutHeaders = {
            ...
            ...Object.fromEntries(Object.entries(headers).map(([header, value])=>[
                    header,
                    [
                        value
                    ]
            ...
            ...telemetry ? Object.fromEntries(Object.entries(getTelemetryHeaders(telemetry)).map(([header, value])=>[
                    header,
                    [
                        value
                    ]
...
var LazyInvokeStep = class extends BaseLazyStep {
   ...
   */ workflowId;
    constructor(stepName, { workflow, body, headers = {}, workflowRunId, retries, retryDelay, flowControl }){
        super(stepName);
        this.params = {
            workflow,
            body,
   ...
   */ getResultStep(concurrent, stepId) {
    ...
    getBody({ context, step, telemetry, invokeCount }) {
        ...
        const request = {
            ...
            headers: Object.fromEntries(Object.entries(invokerHeaders).map((pairs)=>[
                    pairs[0],
                    [
                        pairs[1]
                    ]
    ...
    getHeaders({ context, telemetry, invokeCount }) {
        const { workflow, headers = {}, workflowRunId = getWorkflowRunId(), retries, retryDelay, flowControl } = this.params;
        const newUrl = context.url.replace(/[^/]+$/, this.workflowId);
        const { retries: workflowRetries, retryDelay: workflowRetryDelay, failureFunction, failureUrl, useJSONContent, flowControl: workflowFlowControl } = workflow.options;
...
var WorkflowHeaders = class {
    ...
    stepInfo;
    headers;
    constructor({ userHeaders, workflowConfig, invokeCount, initHeaderValue, stepInfo }){
    ...
    addBaseHeaders() {
        this.headers.rawHeaders = {
            ...
            ...this.workflowConfig.telemetry && this.stepInfo?.lazyStep instanceof LazyCallStep && this.stepInfo.lazyStep.headers[AGENT_NAME_HEADER] ? {
    ...
    addFlowControl() {
        ...
        const { flowControlKey, flowControlValue } = prepareFlowControl(this.workflowConfig.flowControl);
    ...
    addContentType() {
        ...
        const callHeaders = new Headers(this.stepInfo?.lazyStep instanceof LazyCallStep ? this.stepInfo.lazyStep.headers : {});
...
var prepareFlowControl = (flowControl)=>{
    ...
    if (controlValue.length === 0) {
        throw new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$qstash$2f$chunk$2d$H3E2J2K6$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["QstashError"]("Provide at least one of parallelism or ratePerSecond for flowControl");
    }
    return {
...
var submitParallelSteps = async ({ context, steps, initialStepCount, invokeCount, telemetry, debug })=>{
    const planSteps = steps.map((step, index)=>step.getPlanStep(steps.length, initialStepCount + index));
    await debug?.log("SUBMIT", "SUBMIT_STEP", {
        length: planSteps.length,
        steps: planSteps
    });
    const result = await context.qstashClient.batch(planSteps.map((planStep)=>{
        const { headers } = getHeaders({
            initHeaderValue: "false",
            workflowConfig: {
                workflowRunId: context.workflowRunId,
...
var submitSingleStep = async ({ context, lazyStep, stepId, invokeCount, concurrency, telemetry, debug })=>{
...
var AutoExecutor = class _AutoExecutor {
   ...
   */ async addStep(stepInfo) {
        if (this.executingStep) {
            throw new WorkflowError(`A step can not be run inside another step. Tried to run '${stepInfo.stepName}' inside '${this.executingStep}'`);
        }
        this.stepCount += 1;
   ...
   */ async runParallel(parallelSteps) {
        ...
        if (parallelCallState !== "first" && plannedParallelStepCount !== parallelSteps.length) {
            throw new WorkflowError(`Incompatible number of parallel steps when call state was '${parallelCallState}'. Expected ${parallelSteps.length}, got ${plannedParallelStepCount} from the request.`);
        }
            ...
            case "partial":
                {
                    ...
                    if (!planStep || planStep.targetStep === void 0) {
                        throw new WorkflowError(`There must be a last step and it should have targetStep larger than 0.Received: ${JSON.stringify(planStep)}`);
                    }
                    ...
                    try {
                        ...
                        if (error instanceof WorkflowAbort || error instanceof __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$qstash$2f$chunk$2d$H3E2J2K6$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["QstashError"] && error.status === 400) {
                            throw error;
                        }
                        throw new WorkflowError(`Error submitting steps to QStash in partial parallel step execution: ${error}`);
                    }
                    break;
                }
            case "discard":
            ...
            case "last":
                {
                    ...
                    return parallelResultSteps.map((step, index)=>parallelSteps[index].parseOut(step.out));
                }
        }
   ...
   */ // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-parameters
    static getResult(lazyStepList, result, index) {
        if (lazyStepList.length === 1) {
            ...
            throw new WorkflowError(`Unexpected parallel call result while executing step ${index}: '${result}'. Expected ${lazyStepList.length} many items`);
        }
    }
...
var validateStep = (lazyStep, stepFromRequest)=>{
    if (lazyStep.stepName !== stepFromRequest.stepName) {
        throw new WorkflowError(`Incompatible step name. Expected '${lazyStep.stepName}', got '${stepFromRequest.stepName}' from the request`);
    }
    if (lazyStep.stepType !== stepFromRequest.stepType) {
        throw new WorkflowError(`Incompatible step type. Expected '${lazyStep.stepType}', got '${stepFromRequest.stepType}' from the request`);
    }
};
var validateParallelSteps = (lazySteps, stepsFromRequest)=>{
    try {
        ...
        if (error instanceof WorkflowError) {
            ...
            throw new WorkflowError(`Incompatible steps detected in parallel execution: ${error.message}
...
var AnthropicAPI = class extends BaseWorkflowApi {
    async call(stepName, settings) {
        ...
        return await this.callApi(stepName, {
            api: {
                name: "llm",
                provider: (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$qstash$2f$chunk$2d$H3E2J2K6$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["anthropic"])({
                    token
                })
            },
            ...parameters
...
var OpenAIAPI = class extends BaseWorkflowApi {
    async call(stepName, settings) {
        ...
        const provider = useOpenAI ? (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$qstash$2f$chunk$2d$H3E2J2K6$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["openai"])({
            token,
            organization
        }) : (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$qstash$2f$chunk$2d$H3E2J2K6$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["custom"])({
            baseUrl: baseURL,
            token
        });
        return await this.callApi(stepName, {
...
var Agent = class {
   ...
   */ async call({ prompt }) {
        try {
            ...
            if (error instanceof __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ai$2f$dist$2f$index$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__$3c$locals$3e$__["ToolExecutionError"]) {
                if (error.cause instanceof Error && error.cause.name === "WorkflowAbort") {
                    throw error.cause;
                } else if (error.cause instanceof __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ai$2f$dist$2f$index$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__$3c$locals$3e$__["ToolExecutionError"] && error.cause.cause instanceof Error && error.cause.cause.name === "WorkflowAbort") {
   ...
   */ asTool() {
        ...
        return (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$ai$2f$dist$2f$index$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__$3c$locals$3e$__["tool"])({
            parameters: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zod$2f$v3$2f$external$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__$3c$export__$2a$__as__z$3e$__["z"].object({
                prompt: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zod$2f$v3$2f$external$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__$3c$export__$2a$__as__z$3e$__["z"].string()
            }),
...
var ManagerAgent = class extends Agent {
   ...
   */ constructor({ agents, background = MANAGER_AGENT_PROMPT, model, maxSteps, name = "manager llm" }, context){
        super({
            background,
            maxSteps,
            tools: Object.fromEntries(agents.map((agent)=>[
                    agent.name,
                    agent.asTool()
                ])),
            name,
...
var Task = class {
   ...
   */ async run() {
        ...
        if ("agent" in otherParams) {
            ...
            const result = await managerAgent.call({
                prompt
            });
            return {
                text: result.text
...
var WorkflowAgents = class {
   ...
   */ openai(...params) {
        ...
        const openaiModel = this.AISDKModel({
            context: this.context,
            provider: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$ai$2d$sdk$2f$openai$2f$dist$2f$index$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["createOpenAI"],
            providerParams: {
                baseURL,
                apiKey,
...
var WorkflowContext = class {
   ...
   */ async sleepUntil(stepName, datetime) {
    ...
    async call(stepName, settings) {
        ...
        if ("workflow" in settings) {
            ...
            callStep = new LazyCallStep(stepName, url, "POST", settings.body, settings.headers || {}, settings.retries || 0, settings.retryDelay, settings.timeout, settings.flowControl ?? settings.workflow.options.flowControl);
        } else {
            const { url, method = "GET", body, headers = {}, retries = 0, retryDelay, timeout, flowControl } = settings;
            callStep = new LazyCallStep(stepName, url, method, body, headers, retries, retryDelay, timeout, flowControl);
        }
   ...
   */ async notify(stepName, eventId, eventData) {
        return await this.addStep(new LazyNotifyStep(stepName, eventId, eventData, this.qstashClient.http));
    }
...
var DisabledWorkflowContext = class _DisabledWorkflowContext extends WorkflowContext {
   ...
   */ static async tryAuthentication(routeFunction, context) {
        const disabledContext = new _DisabledWorkflowContext({
            qstashClient: new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$qstash$2f$chunk$2d$H3E2J2K6$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["Client"]({
...
var validateRequest = (request)=>{
    ...
    if (!isFirstInvocation && versionHeader !== WORKFLOW_PROTOCOL_VERSION) {
        throw new WorkflowError(`Incompatible workflow sdk protocol version. Expected ${WORKFLOW_PROTOCOL_VERSION}, got ${versionHeader} from the request.`);
    }
...
var parseRequest = async (requestPayload, isFirstInvocation, workflowRunId, requester, messageId, debug)=>{
    if (isFirstInvocation) {
        ...
        let rawSteps;
        if (!requestPayload) {
            await debug?.log("INFO", "ENDPOINT_START", "request payload is empty, steps will be fetched from QStash.");
            const { steps: fetchedSteps, workflowRunEnded } = await getSteps(requester, workflowRunId, messageId, debug);
            if (workflowRunEnded) {
                return {
...
var handleFailure = async (request, requestPayload, qstashClient, initialPayloadParser, routeFunction, failureFunction, env, retries, retryDelay, flowControl, debug)=>{
    ...
    if (!failureFunction) {
        return err(new WorkflowError("Workflow endpoint is called to handle a failure, but a failureFunction is not provided in serve options. Either provide a failureUrl or a failureFunction."));
    }
    try {
...
var processOptions = (options)=>{
    ...
    return {
        ...
        receiver: receiverEnvironmentVariablesSet ? new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$qstash$2f$chunk$2d$H3E2J2K6$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["Receiver"]({
...
var determineUrls = async (request, url, baseUrl, failureFunction, failureUrl, debug)=>{
    ...
    if (!(workflowUrl.startsWith("http://") || workflowUrl.startsWith("https://"))) {
        throw new WorkflowError(`Workflow URL should start with 'http://' or 'https://'. Recevied is '${workflowUrl}'`);
    }
    return {
        workflowUrl,
        workflowFailureUrl
...
var serveBase = (routeFunction, telemetry, options)=>{
    const { qstashClient, onStepFinish, initialPayloadParser, url, verbose, receiver, failureUrl, failureFunction, baseUrl, env, retries, retryDelay, useJSONContent, disableTelemetry, flowControl, onError } = processOptions(options);
    telemetry = disableTelemetry ? void 0 : telemetry;
    const debug = WorkflowLogger.getLogger(verbose);
    const handler = async (request)=>{
        await debug?.log("INFO", "ENDPOINT_START");
        const { workflowUrl, workflowFailureUrl } = await determineUrls(request, url, baseUrl, failureFunction, failureUrl, debug);
        const requestPayload = await getPayload(request) ?? "";
        await verifyRequest(requestPayload, request.headers.get("upstash-signature"), receiver);
        const { isFirstInvocation, workflowRunId } = validateRequest(request);
        debug?.setWorkflowRunId(workflowRunId);
        const { rawInitialPayload, steps, isLastDuplicate, workflowRunEnded } = await parseRequest(requestPayload, isFirstInvocation, workflowRunId, qstashClient.http, request.headers.get("upstash-message-id"), debug);
        if (workflowRunEnded) {
        ...
        const failureCheck = await handleFailure(request, requestPayload, qstashClient, initialPayloadParser, routeFunction, failureFunction, env, retries, retryDelay, flowControl, debug);
        ...
        if (authCheck.isErr()) {
            ...
            await debug?.log("ERROR", "ERROR", {
                error: AUTH_FAIL_MESSAGE
            });
            return onStepFinish(isFirstInvocation ? "no-workflow-id" : workflowContext.workflowRunId, "auth-fail", {
                condition: "auth-fail"
            });
        }
...
var serve = (routeFunction, options)=>{
    return serveBase(routeFunction, {
        sdk: SDK_TELEMETRY,
        framework: "unknown"
    }, options);
};
...
"[project]/node_modules/@upstash/workflow/index.mjs [app-rsc] (ecmascript) <locals>": ((__turbopack_context__) => {
"use strict";

__turbopack_context__.s({
    "Client": ()=>Client
});
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$workflow$2f$chunk$2d$RP7G4UD5$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@upstash/workflow/chunk-RP7G4UD5.mjs [app-rsc] (ecmascript)");
// src/client/index.ts
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$qstash$2f$index$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__$3c$module__evaluation$3e$__ = __turbopack_context__.i("[project]/node_modules/@upstash/qstash/index.mjs [app-rsc] (ecmascript) <module evaluation>");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$qstash$2f$chunk$2d$H3E2J2K6$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/@upstash/qstash/chunk-H3E2J2K6.mjs [app-rsc] (ecmascript)");
;
...
var DLQ = class _DLQ {
   ...
   */ async retryFailureFunction({ dlqId }) {
    ...
    static handleDLQOptions(options) {
        ...
        const headers = {};
        if (flowControl) {
            const { flowControlKey, flowControlValue } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$workflow$2f$chunk$2d$RP7G4UD5$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["prepareFlowControl"])(flowControl);
            headers["Upstash-Flow-Control-Key"] = flowControlKey;
            headers["Upstash-Flow-Control-Value"] = flowControlValue;
        }
        if (retries !== void 0) {
    ...
    static getDlqIdQueryParameter(dlqId) {
        const dlqIds = Array.isArray(dlqId) ? dlqId : [
            dlqId
        ];
        const paramsArray = dlqIds.map((id)=>[
                "dlqIds",
                id
            ]);
...
var Client = class {
    ...
    constructor(clientConfig){
        if (!clientConfig?.token) {
            console.error("QStash token is required for Upstash Workflow!\n\nTo fix this:\n1. Get your token from the Upstash Console (https://console.upstash.com/qstash)\n2. Initialize the workflow client with:\n\n   const client = new Client({\n     token: '<YOUR_QSTASH_TOKEN>'\n   });");
        }
        this.client = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$qstash$2f$chunk$2d$H3E2J2K6$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["Client"](clientConfig);
    }
    /**
   ...
   */ async cancel({ ids, urlStartingWith, all }) {
        let body;
        if (ids) {
            const runIdArray = typeof ids === "string" ? [
                ids
            ] : ids;
            body = JSON.stringify({
                workflowRunIds: runIdArray
            });
        } else if (urlStartingWith) {
            body = JSON.stringify({
                workflowUrl: urlStartingWith
            });
        } else if (all) {
            body = "{}";
        } else {
   ...
   */ async notify({ eventId, eventData }) {
        return await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$workflow$2f$chunk$2d$RP7G4UD5$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["makeNotifyRequest"])(this.client.http, eventId, eventData);
    }
    /**
   ...
   */ async getWaiters({ eventId }) {
        return await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$workflow$2f$chunk$2d$RP7G4UD5$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["makeGetWaitersRequest"])(this.client.http, eventId);
    }
    async trigger(params) {
        const isBatchInput = Array.isArray(params);
        const options = isBatchInput ? params : [
            params
        ];
        const invocations = options.map((option)=>{
            const failureUrl = option.useFailureFunction ? option.url : option.failureUrl;
            const finalWorkflowRunId = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$workflow$2f$chunk$2d$RP7G4UD5$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["getWorkflowRunId"])(option.workflowRunId);
            const context = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$workflow$2f$chunk$2d$RP7G4UD5$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["WorkflowContext"]({
                ...
                telemetry: {
                    sdk: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$workflow$2f$chunk$2d$RP7G4UD5$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["SDK_TELEMETRY"]
                },
        ...
        const result = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$upstash$2f$workflow$2f$chunk$2d$RP7G4UD5$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["triggerFirstInvocation"])(invocations);
        const workflowRunIds = invocations.map((invocation)=>invocation.workflowContext.workflowRunId);
        if (result.isOk()) {
            return isBatchInput ? workflowRunIds.map((id)=>({
                    workflowRunId: id
                })) : {
...
"[project]/node_modules/@upstash/workflow/index.mjs [app-rsc] (ecmascript) <module evaluation>": ((__turbopack_context__) => {
"use strict";
...
//# sourceMappingURL=node_modules_%40upstash_workflow_326abbb4._.js.map